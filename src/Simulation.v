...
Section InvRSim.
  ...
  Proof.
    ...
    assert (ginv st2) by (eapply Hinv; econstructor; eauto).
    assert (ginv st3).
    { eapply Hinv.
      eapply reachable_steps;
        [|eapply StepsCons; [eapply StepsNil|eassumption]].
      econstructor; eauto.
    }
  ...
  Theorem invRSim_implies_refinement:
    (steps stepI) # (steps stepS) |-- impl ⊑ spec.
  Proof.
    unfold Simulates, Refines; intros.
    inv H3.
    eapply invR_simulation_steps in H4; [|reflexivity].
    destruct H4 as [sst2 [shst [? [? ?]]]].
    econstructor; eauto.
  Qed.
