...
Require Import Ex.MsiInc.MsiInvOk.
...
Section Sim.
  ...
  Section ObjCoh.
    ...
    Definition cohMsgs: list (MSig * (Id Msg -> Prop)) :=
      (| (downTo cidx, (MRs, msiRsS)): fun idm => (valOf idm).(msg_value) = cv
       | (rsUpFrom cidx, (MRs, msiDownRsS)): fun idm => (valOf idm).(msg_value) = cv
       | (rsUpFrom cidx, (MRs, msiDownRsIM)): fun idm => (valOf idm).(msg_value) = cv)%cases.
  ...
  Section ObjCohFacts.
    ...
    Lemma MsgsCoh_other_msg_id_enqMP:
      forall cv cidx msgs,
        MsgsCoh cv cidx msgs ->
        forall midx msg,
          ~ In (msg_id msg) [msiRsS; msiDownRsS; msiDownRsIM] ->
          MsgsCoh cv cidx (enqMP midx msg msgs).
    Proof.
  ...
  Ltac solve_ImplStateCoh := idtac.
  Ltac solve_SpecStateCoh := eapply SimStateIntro; [solve_rule_conds_ex|].
  Ltac solve_sim_msi_ext_mp := red; simpl; split; [|solve_sim_ext_mp].
  ...
  Proof. (* SKIP_PROOF_ON
    ...
    inv H2; [apply msi_sim_silent; assumption
    ...
    - (*! Cases for Li caches *)
      ...
      { (* [liDownIRsUpUpMS] *)
        ...
        derive_footprint_info_basis oidx;
          [disc_MsiDownLockInv oidx H29|].
        (** No [solve_NoRsI_..] tactics work here.. *)
        assert (NoRsI oidx msgs).
        { solve_NoRsI_base.
          assert ((orqs +[oidx <- porq-[ downRq]])@[oidx] = Some (porq-[downRq]))
            as Hnorq by mred.
          clear Hpmcf.
          assert (InMPI (enqMP (rsUpFrom oidx)
                               {| msg_id := msiDownRsIM;
                                  msg_type := MRs;
          ...
          clear H37.
          disc_MsgConflictsInv oidx; clear Hnorq.
          specialize (H37 (downTo oidx, msg0) eq_refl H41 H38); dest.
          move H51 at bottom.
          eapply H51 with (rsUp := (rsUpFrom oidx, _)); eauto.
          apply InMP_or_enqMP; left; split; reflexivity.
        }
        disc_InvDirS oidx.
        (** [derive_coherence_of oidx] does not work here.. *)
        specialize (H30 ltac:(solve_msi) H37).
        disc_responses_from.
        solve_sim_msi.
      }
