...
Require Import Ex.MsiInc.MsiInv Ex.MsiInc.MsiInvInv0.
...
Definition InvDirS (st: State): Prop :=
  forall oidx,
    ost <+- (st_oss st)@[oidx]; (ost#[dir].(dir_st) = msiS -> ost#[status] = msiS).
...
Section InvDirS.
  ...
  Lemma msi_InvDirS_init:
    Invariant.InvInit impl InvDirS.
  Proof.
  ...
  Proof. (* SKIP_PROOF_ON
    ...
    - (*! Cases for the main memory *)
      ...
      all: try (red; simpl; intros; repeat (red; simpl; mred); apply H0; fail).
    ...
    - (*! Cases for Li caches *)
      ...
      1: { (** Rules per a child *)
        ...
        all: try (red; simpl; intros; repeat (red; simpl; mred); apply H0; fail).
        all: try (replace (oss +[oidx <- pos]) with oss by meq; assumption).
        { red; simpl; intros.
          repeat (red; simpl; mred).
          { simpl; intros.
            specialize (H0 oidx); simpl in H0; mred.
            apply H0.
            eapply getDir_S_imp; eassumption.
          }
          { apply H0. }
        }
        { red; simpl; intros.
      ...
      { (* [msiInvRsDownDown]; [InvWBDir] is used here. *)
        ...
        red; simpl; intros.
        repeat (red; simpl; mred).
        { simpl; intros; simpl in Hwd; solve_msi. }
        { apply H0. }
      }
  ...
  Theorem msi_InvDirS_ok:
    InvReachable impl step_m InvDirS.
  Proof.
    eapply inv_reachable.
    - typeclasses eauto.
    - apply msi_InvDirS_init.
    - apply msi_InvDirS_step.
  Qed.
...
Section InvDirM.
  ...
  Proof. (* SKIP_PROOF_ON
    ...
    pose proof (msi_InvL1DirI_ok H) as Hl1d.
    pose proof (msi_InvDirS_ok H) as Hdirs.
    inv H1; [assumption
    ...
    - (*! Cases for Li caches *)
      ...
      { (* [liGetMRsDownRqDownDirS] *)
        ...
        { split.
          ...
          { red; simpl; intros; intuition solve_msi. }
        }
        { disc_ObjDirM; mred. }
        { solve_valid. }
      }
...
Section InvWB.
  ...
  Proof.
    ...
    red in H9.
    assert (ost#[dir].(dir_st) <= msiS) as Hdi by solve_msi.
    specialize (H9 Hdi).
