  ...
  Section Rules.
    ...
    Section Li.
      ...
      Definition liDownIRsUpUpM: Rule :=
        rule 1~>10~>1 from template rsuuo {
          receive msiDownRsIM;
          hold msiDownRqIM;
          assert (fun ost orq mins => ost#[dir].(dir_st) = msiM);
          (!|ost, idm, rq, rsbTo|
      ...
      Definition liBInvRqS: Rule :=
        rule 3~>0~>0 from template rqsd {
          assert
            (fun ost =>
               ost#[dir].(dir_sharers) <> nil /\
               SubList ost#[dir].(dir_sharers) (subtreeChildrenIndsOf topo oidx) /\
               ost#[dir].(dir_st) = msiS);
          (ost --> (ost#[dir].(dir_sharers), <| msiDownRqIS; O |>))
        }.

      Definition liBInvRqM: Rule :=
        rule 3~>0~>1 from template rqsd {
          assert
            (fun ost =>
               In ost#[dir].(dir_excl) (subtreeChildrenIndsOf topo oidx) /\
               ost#[dir].(dir_st) = msiM);
          (ost --> ([ost#[dir].(dir_excl)], <| msiDownRqIM; O |>))
        }.

      Definition liBInvRsS0: Rule :=
        rule 3~>1~>0 from template rsus {
          receive msiDownRsIS;
          assert (fun ost orq mins => ost#[owned] = false /\ ost#[dir].(dir_st) = msiS);
          (!|ost, _| --> (ost +#[dir <- setDirI]))
        }.

      Definition liBInvRsS1: Rule :=
        rule 3~>1~>1 from template rsus {
          receive msiDownRsIS;
          assert (fun ost orq mins => ost#[owned] = true /\ ost#[dir].(dir_st) = msiS);
          (!|ost, _| --> (ost +#[dir <- setDirI]
                              +#[status <- msiM]))
        }.

      Definition liBInvRsM: Rule :=
        rule 3~>1~>2 from template rsuso {
          receive msiDownRsIM;
          assert (fun ost orq mins => ost#[dir].(dir_st) = msiM);
          (!|ost, idm| --> (ost +#[dir <- setDirI]
                                +#[owned <- true]
                                +#[status <- msiM]
                                +#[val <- msg_value (valOf idm)]))
        }.
