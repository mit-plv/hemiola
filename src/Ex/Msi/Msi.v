  ...
  Section Rules.
    ...
    Section L1.
      ...
      Definition l1InvRqUpUp: Rule :=
        ...
        :requires
           (fun ost mins =>
              ost#[owned] = false /\ msiNP < ost#[status] < msiM)
        :transition
           (ost --> <| msiInvRq; O |>).
    ...
    Section Li.
      ...
      Definition liDownSRsUpDownM: Rule :=
        ...
        :holding msiRqS
        :requires
           (fun ost orq mins => ost#[dir].(dir_st) = msiM /\ FirstMsg ost orq mins)
        :transition
           (!|ost, idm, rq, rsbTo|
            --> (ost +#[owned <- true]
      ...
      Definition liGetMRqUpDownM: Rule :=
        ...
        :requires
           (fun ost mins =>
              ...
              ost#[status] <= msiI /\ ost#[dir].(dir_st) = msiM)
        :transition
           (!|ost, msg| --> ([ost#[dir].(dir_excl)], <| msiDownRqI; O |>)).
      ...
      Definition liDownIImm: Rule :=
        ...
        :me oidx
        :requires (fun ost orq mins =>
                     msiS <= ost#[status] /\ ost#[dir].(dir_st) = msiI)
        :transition
           (!|ost, min| --> (ost +#[owned <- false]
                                 +#[status <- msiI],
                             <| msiDownRsI; O |>)).
      ...
      Definition liPushImm: Rule :=
        ...
        :requires
           (fun ost orq mins =>
              msiI < ost#[dir].(dir_st) /\
              ost#[status] = msiS /\ ost#[owned] = false)
      ...
      {| obj_idx := oidx;
         obj_rules :=
           (memRulesFromChildren (subtreeChildrenIndsOf topo oidx))
             ++ [liDownSRsUpDownM; liDownIRsUpDown];
         obj_rules_valid := _ |}.
    Next Obligation.
      solve_inds_NoDup disc_child_inds_disj.
    Qed.
...
Hint Unfold l1GetSImm l1GetSRqUpUp l1GetSRsDownDown
     ...
     l1DownIImm l1InvRqUpUp l1InvRqUpUpWB l1InvRsDownDown: MsiRules.
