...
Section Footprints.
  ...
  Definition MsiUpLockInv (st: MState): Prop :=
    forall oidx,
      ost <+- (bst_oss st)@[oidx];
  ...
  Definition MsiDownLockInv (st: MState): Prop :=
    forall oidx,
      ost <+- (bst_oss st)@[oidx];
        orq <+- (bst_orqs st)@[oidx];
        rqid <+- orq@[downRq];
...
Section FootprintsOk.
  ...
  Lemma MsiUpLockInv_update_None:
    forall oss orqs pmsgs,
      MsiUpLockInv {| bst_oss := oss;
                       bst_orqs := orqs;
                       bst_msgs := pmsgs |} ->
      forall oidx nost norq nmsgs,
        norq@[upRq] = None ->
        MsiUpLockInv {| bst_oss := oss +[oidx <- nost];
  ...
  Lemma MsiDownLockInv_update_None:
    forall oss orqs pmsgs,
      MsiDownLockInv topo {| bst_oss := oss;
                              bst_orqs := orqs;
                              bst_msgs := pmsgs |} ->
      forall oidx nost norq nmsgs,
        norq@[downRq] = None ->
        MsiDownLockInv topo {| bst_oss := oss +[oidx <- nost];
                                bst_orqs := orqs +[oidx <- norq];
                                bst_msgs := nmsgs |}.
  Proof.
  ...
  Proof. (* SKIP_PROOF_OFF *)
    ...
    - (*! Cases for the main memory *)
      ...
      dest_in; disc_rule_conds_ex.
      all: try (eapply MsiDownLockInv_update_None; eauto; mred).
