...
Lemma MsiHOStateIfc_host_ty_ok:
  forall i: Fin.t ost_sz,
    match Vector.nth [Some hdv_type; Some HBool; Some HMsi; None]%vector i with
...
Instance MsiHOStateIfc: HOStateIfc :=
  {| host_ty := [Some hdv_type; Some HBool; Some HMsi; None]%vector;
     host_ty_ok := MsiHOStateIfc_host_ty_ok;
  |}.
...
Lemma MsiHOStateIfcFull_hostf_ty_compat:
  forall i hbt,
    host_ty[@i] = Some hbt ->
    [HBType hdv_type; HBType HBool; HBType HMsi; HEType HDir][@i] = HBType hbt.
Proof.
  intros i.
  refine (match i with | Fin.F1 => _ | Fin.FS _ => _ end);
    [repeat (destruct n; [exact idProp|]);
...
Instance MsiHOStateIfcFull: HOStateIfcFull :=
  {| hostf_ty := [HBType hdv_type; HBType HBool; HBType HMsi; HEType HDir];
     hostf_ty_ok := eq_refl;
     hostf_ty_compat := MsiHOStateIfcFull_hostf_ty_compat;
  |}.
...
Ltac renote_rule_init :=
  ...
  intros; repeat autounfold with MsiRules;
...
Section Deep.
  ...
  Section Object.
    ...
    Definition hl1: HObject (Msi.l1 oidx).
    Proof.
    ...
    Definition hli: HObject (MsiImp.li tr oidx).
    Proof.
      refine {| hobj_rules_ok := _ |}.
      cbv [MsiImp.li obj_rules].
      repeat match goal with
             | |- map _ _ = (_ ++ _)%list =>
    ...
    Definition hmem: HObject (MsiImp.mem tr oidx).
    Proof.
      refine {| hobj_rules_ok := _ |}.
      cbv [MsiImp.mem obj_rules].
      repeat match goal with
             | |- map _ _ = (_ ++ _)%list =>
               instantiate (1:= (_ ++ _)%list); rewrite map_app; simpl; f_equal
             | |- map _ _ = (_ :: _)%list => instantiate (1:= (_ :: _)%list); simpl; f_equal
  ...
  Definition himpl: HSystem (MsiImp.impl Htr).
  Proof.
    refine {| hsys_objs_ok := _ |}.
    cbv [MsiImp.impl sys_objs].
